 ...
 */
function meoswetkit_install_tasks(&$install_state) {
  ...
  // Add the MEOS WetKit theme selection to the installation process.
  require_once drupal_get_path('module', 'wetkit_theme') . '/meoswetkit_theme.profile.inc';
  $tasks = $tasks + meoswetkit_theme_profile_theme_selection_install_task($install_state);

  // Set up a task to include secondary language (fr).
  $tasks['meoswetkit_batch_processing'] = array(
    'display_name' => st('Import French Language'),
    'type' => 'batch',
  );

  $tasks['meoswetkit_import_content'] = array(
  ...
  if (isset($install_state['parameters']['demo_content'])) {
    $tasks['meoswetkit_import_demo_content'] = array(
...
function meoswetkit_install_tasks_alter(&$tasks, $install_state) {
  ...
  _meoswetkit_set_theme('meos_wetkit_shiny');
  ...
  require_once drupal_get_path('module', 'meoswetkit_core') . '/meoswetkit_core.profile.inc';
  $tasks['install_load_profile']['function'] = 'meoswetkit_core_install_load_profile';
}
 ...
 */
function meoswetkit_install_welcome_submit($form, &$form_state) {
 ...
 */
function meoswetkit_form_install_configure_form_alter(&$form, $form_state) {
  // Hide some messages from various modules that are just too chatty.
  drupal_get_messages('status');
  drupal_get_messages('warning');
  ...
  $form['meoswetkit_settings'] = array(
    '#type' => 'fieldset',
    '#title' => st('WXT Settings'),
  );

  $form['meoswetkit_settings']['demo_content'] = array(
    '#title' => st('Import Demo Content'),
    '#description' => st('Whether demo content should imported.'),
    '#type' => 'checkbox',
    '#default_value' => drupal_is_cli() ? FALSE : TRUE,
  );
  array_push($form['#submit'], 'meoswetkit_import_demo_content_form_submit');
}

/**
 * Batch Processing for French Language import.
 */
function meoswetkit_batch_processing(&$install_state) {
 ...
 */
function meoswetkit_import_content() {

  // Run Mega Menu migration.
  $operations[] = array('_meoswetkit_import', array(
    'WetKitMigrateMegaMenu',
    t('Importing content.'),
    ));

  // Run entities import.
  $operations[] = array('_meoswetkit_entities_import', array(
    t('Initializing entities.'),
    ));

  // Page Manager Fix.
  $operations[] = array('_meoswetkit_panels_fix', array(
    t('Fix Page Manager dependency chain issue.'),
    ));

  // Permissions Fix.
  $operations[] = array('_meoswetkit_permissions_fix', array(
  ...
  $batch = array(
    ...
    'file' => drupal_get_path('profile', 'meoswetkit') . '/meoswetkit.install_callbacks.inc',
  );
  return $batch;
}
 ...
 */
function meoswetkit_import_demo_content_form_submit($form, &$form_state) {
  global $install_state;
 ...
 * Task callback: return a batch API array with the products to be imported.
 */
function meoswetkit_import_demo_content() {
  ...
  $operations[] = array('_meoswetkit_bean_import', array(t('Importing Bean content.')));
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function meoswetkit_form_apps_profile_apps_select_form_alter(&$form, $form_state) {
  ...
  if (isset($form['apps_fieldset'])) {
    $manifest = apps_manifest(apps_servers('meoswetkit'));
